...
class AddFilm(FlaskForm):
    title = StringField(label="Film title", validators=[InputRequired(message="Title not specified."),
                                                        Length(min=2, max=64,
                                                               message="Title must be 2 to 64 characters long.")])
    release_date = DateField(label="Release date", validators=[InputRequired(message="Release date not specified.")])
    description = TextAreaField(label="Description", validators=[Length(max=2048, message="Description is too long.")])
    rating = DecimalField(label="Rating", places=2, validators=[NumberRange(
                                                                min=0.0,
                                                                max=10.0,
